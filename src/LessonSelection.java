...
public class LessonSelection extends JFrame implements ActionListener {
	// declare swing widgets
	private JLabel frameBackground = new JLabel(new ImageIcon("frameimages/lessonSelection.png"));
	...
	// constructor method
	public LessonSelection(int lessonCompleted, int activityCompleted) {
	...
	private void placeButtons() {
		...
		for (int i = 0; i < buttonArray.length; i++) {
			...
			buttonArray[i].setBounds(162 + i * 340, 489, 196, 54);
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == buttonArray[0]) {
			// if the previous lesson is completed, let user complete the next lesson
			if ((lessonCompleted == 0 && activityCompleted == 0) || (lessonCompleted == 1 && activityCompleted == 0)
					|| (lessonCompleted == 3 && activityCompleted == 3)) {
				new Lesson1(lessonCompleted, activityCompleted);
				dispose();
			}
			// else wise, tell user they have completed the lesson already
			else {
				JOptionPane.showMessageDialog(this, "You have already completed this lesson!", "Lesson Completed Prior",
						JOptionPane.INFORMATION_MESSAGE);
			}
		} else if (e.getSource() == buttonArray[1]) {
			// lesson 1 is completed let user access lesson 2
			if ((lessonCompleted == 1 && activityCompleted == 1) || (lessonCompleted == 2 && activityCompleted == 1)
					|| (lessonCompleted == 3 && activityCompleted == 3)) {
				new Lesson2(lessonCompleted, activityCompleted);
				dispose();
			}
			// else wise, if the user has not completed the lesson before, tell them
				...
				JOptionPane.showMessageDialog(this,
						...
						"Lesson Locked", JOptionPane.INFORMATION_MESSAGE);
			}
			...
			// lesson 1 is completed let user access lesson 2
			if ((lessonCompleted == 2 && activityCompleted == 2) || (lessonCompleted == 3 && activityCompleted == 2)
					|| (lessonCompleted == 3 && activityCompleted == 3)) {
				new Lesson3(lessonCompleted, activityCompleted);
				dispose();
			}
			// else wise, if the user has not completed the lesson before, tell them
			else if (lessonCompleted < 2 || activityCompleted < 2) {
				JOptionPane.showMessageDialog(this,
